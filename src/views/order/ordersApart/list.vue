        ...
        <div class="join_list" >
          <div class="list-option">
              <div class="page-oper title">
                <div class="page-title">拆单中（当前最多选取30个任务）</div>
             </div>
          </div>
...
export default {
  data() {
    return {
      theadOne: ['颜色', '商品类型', '产品类型', '订单号', '专卖店', '客户名称', '订单信息',
        '订单创建日期'],
      theadTwo: ['颜色', '商品类型', '产品类型', '订单号', '专卖店', '客户名称', '订单信息',
        '提交审核人', '状态'],
      theadThree: ['颜色', '商品类型', '产品类型', '订单号', '专卖店', '客户名称', '订单信息',
        '提交审核日期', '收款状态'],
      tbodyOne: [],
      tbodyTwo: [],
      tbodyThree: [],
  ...
  methods: {
    initAll() {
      ...
      Promise.all([
        Process.orderAparting(),
        Process.orderAparted(),
        Process.orderApartable(),
      ])
        .then(([one, two, three]) => {
    ...
    initOne: function () {
      this.oneLoading = true;
      Process.orderAparting().then(one => {
        this.oneLoading = false;
        this.paginationDataOne = one.data;
        this.tbodyOne = one.data.result;
        this.conditionsOne.pageSize = one.data.pageSize;
    ...
    initThree: function (val) {
      this.threeLoading = true;
      Process.orderApartable(val).then(three => {
        this.threeLoading = false;
        this.paginationDataThree = three.data;
        this.tbodyThree = three.data.result;
        this.conditionsThree.pageSize = three.data.pageSize;
